...
const addOpeningTag = (data, callback) => {
    ...
    db.query(checkSql, [prefixPattern], (checkErr, checkResult) => {
        ...
        const sql = `INSERT INTO opening_tags_entry (
            tag_id, product_id, account_name, invoice, Pricing, cut, color, clarity, 
            subcategory_id, sub_category, design_master, Prefix, category, Purity, metal_type, 
            PCode_BarCode, Gross_Weight, Stones_Weight, deduct_st_Wt, Stones_Price, WastageWeight, 
            HUID_No, Wastage_On, Wastage_Percentage, Weight_BW, MC_Per_Gram, Making_Charges_On, 
            TotalWeight_AW, Making_Charges, rate, tax, tax_amt, total_price, Status, Source, 
            Stock_Point, pieace_cost, product_Name, making_on, selling_price, dropdown, qr_status, 
            stone_price_per_carat, pur_Gross_Weight, pur_Stones_Weight, pur_deduct_st_Wt,
            pur_stone_price_per_carat, pur_Stones_Price, pur_Weight_BW, pur_Making_Charges_On, 
        ...
        const values = insertEntries.map(entry => [
            ...
            entry.invoice, entry.Pricing, entry.cut, entry.color, entry.clarity, 
            entry.subcategory_id, entry.sub_category, entry.design_master, 
            entry.Prefix, entry.category, entry.Purity, entry.metal_type, 
            entry.PCode_BarCode, entry.Gross_Weight, entry.Stones_Weight, 
            entry.deduct_st_Wt, entry.Stones_Price, entry.WastageWeight, 
            entry.HUID_No, entry.Wastage_On, entry.Wastage_Percentage, 
            entry.Weight_BW, entry.MC_Per_Gram, entry.Making_Charges_On, 
            entry.TotalWeight_AW, entry.Making_Charges, entry.rate, entry.tax, 
            entry.tax_amt, entry.total_price, entry.Status, 
            entry.Source, entry.Stock_Point, entry.pieace_cost, entry.product_Name, 
            entry.making_on, entry.selling_price, entry.dropdown, 
            entry.qr_status, entry.stone_price_per_carat, entry.pur_Gross_Weight, 
            entry.pur_Stones_Weight, entry.pur_deduct_st_Wt, 
            entry.pur_stone_price_per_carat, entry.pur_Stones_Price, entry.pur_Weight_BW, 
            entry.pur_Making_Charges_On, entry.pur_MC_Per_Gram,
            entry.pur_Making_Charges, entry.pur_Wastage_On, entry.pur_Wastage_Percentage, 
            entry.pur_WastageWeight, entry.pur_TotalWeight_AW, 
        ...
        db.query(sql, [values], (err, result) => {
            ...
            const updateSql = `UPDATE updated_values_table
                SET bal_pcs = bal_pcs - ?, bal_gross_weight = bal_gross_weight - ?
...
const getAllOpeningTags = (callback) => {
    const sql = `SELECT * FROM opening_tags_entry ORDER BY opentag_id DESC`;
...
const getLastPcode = (callback) => {
    const query = "SELECT PCode_BarCode FROM opening_tags_entry WHERE PCode_BarCode LIKE '0%' ORDER BY opentag_id DESC";
    db.query(query, callback);
};

const getLastPCodeBarCode = (prefix, callback) => {
    const sql = `SELECT PCode_BarCode FROM opening_tags_entry WHERE Prefix = ? ORDER BY opentag_id DESC LIMIT 1`;
