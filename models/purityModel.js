...
const getAllPurities = (callback) => {
  const sql = 'SELECT * FROM purity ORDER BY purity_id DESC';
...
const updatePurityById = (id, data, callback) => {
  const sql = `
    UPDATE purity 
    SET name = ?, metal = ?, purity_percentage = ?, purity = ?, urd_purity = ?, old_purity_desc = ?, cut_issue = ?, skin_print = ?
    WHERE purity_id = ?
  `;
  db.query(sql, [
    data.name, 
